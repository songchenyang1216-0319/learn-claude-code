...
如果你希望按“准备环境 → 运行 → 观察 → 修改 → 验收”的节奏逐课学习，请使用
[S01–S20 逐课实操指南](./guides/zh/)；20 节课程的详细指南均已完成。
...
课程中的 `bash` 工具在 Windows 上会自动调用 Git Bash，不会交给 `cmd.exe`。请先安装 Git for Windows；如果程序找不到 Bash，可在 `.env` 中显式设置：

```dotenv
BASH_EXECUTABLE=C:\Program Files\Git\bin\bash.exe
```

使用商汤 Token Plan 时，在 `.env` 中配置 `LLM_API_STYLE=openai`、`ANTHROPIC_BASE_URL=https://token.sensenova.cn/v1`、API Key 和控制台返回的精确 `MODEL_ID`。主线 20 章和旧版 `agents/` 都会复用同一套 API 与 Shell 适配层。

在新的 Windows 电脑 clone 本仓库后，可以运行一键安装脚本：

```powershell
powershell -ExecutionPolicy Bypass -File .\setup-windows.ps1
```

脚本会创建 `.venv`、安装依赖、永久启用 Python UTF-8、生成本地 `.env` 并验证 Git Bash。API Key 不会被写入 Git 或上传。
