...
from shell_runner import run_bash_command, run_process
...
def run_git(args: list[str]) -> tuple[bool, str]:
    ...
    try:
        r = run_process(["git"] + args, cwd=WORKDIR, timeout=30)
...
def _count_worktree_changes(path: Path) -> tuple[int, int]:
    ...
    try:
        r1 = run_process(["git", "status", "--porcelain"],
                            cwd=path, timeout=10)
        files = len([l for l in r1.stdout.strip().splitlines() if l.strip()])
        r2 = run_process(["git", "log", "@{push}..HEAD", "--oneline"],
                            cwd=path, timeout=10)
...
def run_bash(command: str, cwd: Path = None) -> str:
    return run_bash_command(command, cwd=cwd or WORKDIR)
