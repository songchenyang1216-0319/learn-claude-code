...
from provider_client import create_client
from shell_runner import run_bash_command
import os

load_dotenv()

client = create_client(Anthropic)
MODEL = os.getenv("MODEL_ID", os.getenv("MODEL_NAME", "claude-sonnet-4-20250514"))
    ...
    while True:
        ...
        for b in r.content:
            if b.type == "tool_use":
                ...
                output = run_bash_command(b.input["command"], timeout=60)
...
def execute(name: str, args: dict) -> str:
    ...
    if name == "bash":
        ...
        return run_bash_command(args["command"], cwd=WORKDIR, timeout=60)
...
def create_agent(name: str, level: int, output_dir: Path):
    ...
    # Copy the shared provider and Windows-safe Bash adapters.
    repository_root = Path(__file__).resolve().parents[3]
    for helper_name in ("provider_client.py", "shell_runner.py"):
        helper_source = repository_root / helper_name
        helper_target = agent_dir / helper_name
        shutil.copyfile(helper_source, helper_target)
        print(f"Created: {helper_target}")
