...
On Windows, the course invokes Git Bash explicitly instead of `cmd.exe`. Install Git for Windows; if Bash is not discoverable, set `BASH_EXECUTABLE=C:\Program Files\Git\bin\bash.exe` in `.env`. OpenAI-compatible providers such as SenseNova use the shared `provider_client.py`; see `.env.example` for configuration.

After cloning on another Windows machine, bootstrap the environment with:
